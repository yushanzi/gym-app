...
const PLAN = {
  A: {
    ...
    exercises: [
      ['壶铃罗马尼亚硬拉', '6kg壶铃', '3×12', '微屈膝、髋部向后推，背平直，感受大腿后侧', '壶铃 罗马尼亚硬拉 标准动作'],
      ['椅子深蹲', '自重', '3×10~12', '轻触椅子即起，膝盖对准脚尖，不内扣', '椅子深蹲 正确姿势'],
      ['跪姿俯卧撑', '自重', '3×8~12', '做不了就撑墙/撑桌，躯干一条线', '跪姿俯卧撑 标准动作'],
      ['单臂壶铃划船', '6kg壶铃', '3×12/侧', '背平，肘向后上方拉', '壶铃 单臂划船 正确姿势'],
      ['臀桥', '自重', '3×15', '顶峰停1~2秒，挤压臀部（护膝关键）', '臀桥 标准动作 收紧臀部'],
      ['靠墙静蹲', '自重', '2×30~45秒', '大腿约90°，膝盖不适就调高角度', '靠墙静蹲 正确姿势'],
    ],
  ...
  B: {
    ...
    exercises: [
      ['壶铃摆动', '6kg壶铃', '4组×30秒·休30秒', '髋部发力，膝盖不主动蹲，腰背平直（第2周5组、第4周6组）', '壶铃摆动 标准动作 髋部发力'],
      ['半跪健腹轮', '健腹轮', '3×8~12', '骨盆后倾、收腹、不塌腰，滚出距离先小后大', '健腹轮 跪姿 正确姿势'],
      ['死虫式', '瑜伽垫', '3×10/侧', '下背贴地，对侧手脚慢伸', '死虫式 正确动作 dead bug'],
      ['鸟狗式', '瑜伽垫', '3×10/侧', '躯干稳定不晃动', '鸟狗式 正确动作 bird dog'],
      ['平板支撑', '自重', '3×30~45秒', '臀收紧、不撅臀不塌腰', '平板支撑 正确姿势'],
    ],
    cooldown: '拉伸3分钟',
  },
  C: {
    ...
    exercises: [
      ['哑铃坐姿推举', '2kg×2', '3×15', '核心收紧，不耸肩', '哑铃坐姿推举 标准动作'],
      ['哑铃侧平举', '2kg×2', '3×15', '肘微屈，抬至肩高即可，慢放', '哑铃侧平举 标准动作'],
      ['俯身哑铃划船', '2kg×2', '3×15', '俯身45°，肘贴身体向后拉', '哑铃俯身划船 标准动作'],
      ['哑铃锤式弯举', '2kg×2', '3×15', '肘固定，慢起慢放', '哑铃锤式弯举 标准动作'],
      ['靠墙静蹲', '自重', '3×40秒', '强化股四头，养护膝关节', '靠墙静蹲 正确姿势'],
      ['超人式', '瑜伽垫', '3×12', '强化下背，与健腹轮配套', '超人式 正确动作 superman'],
    ],
    cooldown: '拉伸3分钟',
  },
};
  ...
  return d.exercises
    .map(
      ([n, e, s, tip, q]) => `<li>
        ...
        <a class="vid-link" target="_blank" rel="noopener" href="https://www.youtube.com/results?search_query=${encodeURIComponent(q)}">▶ 跟练视频</a>
      </li>`
    )
    .join('');
}
